---
sidebar_position: 1
description: KnowFlow 分块策略概述，对比智能分块、标题分块、父子分块、正则分块和 Page 按页分块五种策略的适用场景与配置方法。
---
...
KnowFlow 提供五种不同的分块策略,适用于不同类型的文档和使用场景:
...
按照文档物理页生成分块,通常一页对应一个 chunk,适合需要页码级引用和页面级溯源的 PDF 文档。
